...
class WeightedTVLoss(L1Loss):
    ...
    def forward(self, pred, weight=None):
        y_diff = super(WeightedTVLoss,
                       self).forward(pred[:, :, :-1, :],
                                     pred[:, :, 1:, :],
                                     weight=weight[:, :, :-1, :])
        x_diff = super(WeightedTVLoss,
                       self).forward(pred[:, :, :, :-1],
                                     pred[:, :, :, 1:],
                                     weight=weight[:, :, :, :-1])
...
class MultiScaleGANLoss(GANLoss):
    ...
    def __init__(self,
                 ...
                 fake_label_val=0.0,
                 loss_weight=1.0):
        super(MultiScaleGANLoss, self).__init__(gan_type, real_label_val,
                                                fake_label_val, loss_weight)
    ...
    def forward(self, input, target_is_real, is_disc=False):
        ...
        if isinstance(input, list):
            ...
            for pred_i in input:
                ...
                loss_tensor = super().forward(pred_i, target_is_real,
                                              is_disc).mean()
                loss += loss_tensor
...
class GANFeatLoss(nn.Module):
    ...
    def __init__(self, criterion='l1', loss_weight=1.0, reduction='mean'):
        ...
        if criterion == 'l1':
            ...
            raise ValueError(f'Unsupported loss mode: {criterion}. '
    ...
    def forward(self, pred_fake, pred_real):
        ...
        for i in range(num_D):  # for each discriminator
            ...
            for j in range(num_intermediate_outputs):  # for each layer output
                unweighted_loss = self.loss_op(pred_fake[i][j],
...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    fake = Variable(torch.cuda.FloatTensor(real_samples.shape[0], 1, 1,
                                           1).fill_(1.0),
                    requires_grad=False)
...
def lda_loss(x):
    ...
    for i in range(len(x)):
        for j in range(i + 1, len(x)):
            loss_between += torch.mean(
                (x[i] * x[j]) / (torch.sum(x[i]) * torch.sum(x[j]) + 1e-5))
    # loss within
    loss_within = 0
...
class AngularLoss(nn.Module):
    ...
    def __init__(self, *args, loss_fn=F.l1_loss, **kwargs):
        super().__init__(*args, **kwargs)
        self.loss_fn = loss_fn

    def forward(self, gaze, target):
        gaze = F.normalize(gaze)
        target = F.normalize(target)
        total = torch.sum(gaze * target, dim=-1)
        angle = torch.arccos(torch.clip(total, min=-0.9999999, max=0.9999999))
        return self.loss_fn(angle, torch.zeros_like(angle))


class GeLoss(nn.Module):

    def __init__(self, recloss=torch.nn.L1Loss(), w1=1, w2=0.1):
        super(GeLoss, self).__init__()
        self.gloss = AngularLoss()
        self.recloss = recloss
        self.w1 = w1
        self.w2 = w2

    def forward(self, gaze, img, gaze_pre, img_pre):
...
class DeLoss(nn.Module):
    ...
    def __init__(self, recloss=torch.nn.L1Loss()):
        super(DeLoss, self).__init__()
        self.recloss = recloss

    def forward(self, img, img_pre):
...
class QuatLoss(nn.Module):
    ...
    def forward(self, q1, q2):
        ...
        angle_axis = K.quaternion_to_axis_angle(q_rel.data)

        # Extract the angle
        angle = torch.linalg.norm(angle_axis[:, :3], dim=1)
