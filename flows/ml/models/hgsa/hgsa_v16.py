...
# --- Updated HGSABlock with Fixed Anchoring ---
...
class HGSABlock(nn.Module):
    ...
    def forward(self, x, feat, illu_fea, illu_map):
        ...
        for i in range(self.M):
            v = cond_all[:, self.cond_dim * i:self.cond_dim * (i + 1)]
            p_e = self.expert_heads[i](feat, illu_fea,
                                       v).view(B, self.Q, 3, H, W)
...
class HyperExpertHead(nn.Module):
    ...
    def __init__(self, feat_dim, hidden_dim, out_channels, v_dim):
        super().__init__()
        # feat_dim (48) + illu_fea (16) + v_dim (3) = 67
        self.input_proj = nn.Conv2d(feat_dim + 16 + v_dim, hidden_dim, 1)
        self.main_branch = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, groups=hidden_dim),
            nn.SiLU(), nn.Conv2d(hidden_dim, 3 * out_channels, 1))
        self.shortcut = nn.Conv2d(feat_dim + 16 + v_dim, 3 * out_channels, 1)
