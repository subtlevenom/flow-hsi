...
class USGS(nn.Module):
    def __init__(self, M=12, Q=7):
        ...
        self.illum_estimator = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(32, 2, kernel_size=1), # Карта Gain и Bias для каждой точки
        )
        ...
        self.chi_superposition = nn.Sequential(
            nn.Conv2d(Q * 3, Q * 6, kernel_size=3, padding=1, groups=Q), # Расширяем пространство признаков для каждого узла
            nn.SiLU(),
            nn.Conv2d(Q * 6, 64, kernel_size=1), # Смешиваем узлы
            nn.SiLU(),
            nn.Conv2d(64, 3, kernel_size=1) # Финальный RGB остаток
        )
    ...
    def forward(self, x, train=False):
        ...
        sigma = torch.sigmoid(self.enc_sigma(feat)) * 0.45 + 0.05
