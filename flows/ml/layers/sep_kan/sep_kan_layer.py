...
import numpy as np
import torch
from flows.ml.layers.kan.kan_layer import KANLayer


class SepKANLayer(torch.nn.Module):
    ...
    def forward(self, x, w):
        ...
        # 1D and 2D signals
        in_shape = x.shape
        dimentions = list(range(len(in_shape)))
        in_permutation = [0] + dimentions[2:] + [1] # 0, 2...3, 1
        out_permutation = np.argsort(in_permutation).tolist()
        out_shape = [in_shape[i] for i in in_permutation[:-1]] + [self.out_channels]

        # weights (b * h *...* w, kan_size)
        w = w.permute(*in_permutation).reshape(-1, self.size)
        # img (b * h *...* w, kan_size)
        x = x.permute(*in_permutation).reshape(-1, self.in_channels)
        ...
        x = x.view(*out_shape).permute(*out_permutation)
