...
class CDF:
    ...
    def __call__(self, image: np.ndarray, padding: int=0, harmonics=6):
        ...
        center_crop = None
        if padding > 0:
            center_crop = A.CenterCrop(height=image.shape[0], width=image.shape[1])
            pad = A.Pad(padding=padding, border_mode=cv2.BORDER_CONSTANT, fill=0)
            image = pad(image)
        ...
        for channel_index, lens_lambda in enumerate(BANDS):
            temp_lambda = lens_lambda * 1e-9
            ...
            field = channel_image * torch.exp(1j * phase_in)
            ...
            for channel_index in range(image.shape[-1]):
                ...
                if center_crop is not None:
                    intensity = center_crop(image=intensity)['image']

                temp_lambd_cpu = int(temp_lambda * 1e9)
