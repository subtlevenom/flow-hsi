...
def get_filter_bier(path_for_filt='./cmv_400_graph/', show_graph_on=True):
    ...
    if show_graph_on:
        ...
        plt.plot(np.linspace(0, 1000, len(spectral_filters[0])), spectral_filters[0], label='', color='blue')
        plt.plot(np.linspace(0, 1000, len(spectral_filters[1])), spectral_filters[1], label='', color='green')
        plt.plot(np.linspace(0, 1000, len(spectral_filters[2])), spectral_filters[2], label='', color='red')
...
def main_circl(
    ...
    amp_in = load_grayscale(path_to_arc+str(lambda_for_one_lens)+'.png', N)
    ...
    lens_phase = generate_lens_phase(focal_length, temp_lambd, dx_lenz, Nlocal=Nlocal)
    ...
    # phi_lambda = lens_phase * (lambda_for_one_lens * 1e-9 / temp_lambd)
...
if __name__=='__main__':
    ...
    dx_in = 3.0e-6  # шаг пикселя входного распределения
    dx_lenz = 3.0e-6  # шаг линзы
    dx_kam = 3.0e-6  # шаг пикселя камеры

    focal_length = 0.03   # фокусное расстояние линзы (м)
    r_lenz = 0.003
