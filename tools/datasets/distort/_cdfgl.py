...
CMV_400_GRAPH_PATH = '/home/korepanov/work/flow-hsi/tools/datasets/cave_hsi/cmv_400_graph'
...
def read_spectral_filters_from_txt(folder_path):
    ...
    txt_files = [
        str(f) for f in Path(folder_path).glob('**/*') if f.suffix == ".txt"
    ]
...
def get_filter_bier(center_wavelengths_for_graph_baier,
    ...
    if show_graph_on:
        ...
        center_wavelengths_for_graph_baier[0] = round(
            center_wavelengths_for_graph_baier[0] * 1e3, 2)
        center_wavelengths_for_graph_baier[1] = round(
            center_wavelengths_for_graph_baier[1] * 1e3, 2)
        center_wavelengths_for_graph_baier[2] = round(
            center_wavelengths_for_graph_baier[2] * 1e3, 2)

        # print(spectral_filters)
        plt.plot(np.linspace(0, 1000, len(spectral_filters[0])),
                 spectral_filters[0],
                 label=f'{center_wavelengths_for_graph_baier[2]} nm',
                 color='blue')
        plt.plot(np.linspace(0, 1000, len(spectral_filters[1])),
                 spectral_filters[1],
                 label=f'{center_wavelengths_for_graph_baier[1]} nm',
                 color='green')
        plt.plot(np.linspace(0, 1000, len(spectral_filters[2])),
                 spectral_filters[2],
                 label=f'{center_wavelengths_for_graph_baier[0]} nm',
                 color='red')
...
def main_circl(mass_amp_in,
    ...
    for iterator_lenz in range(0, len(lambda_for_lens), 1):
        ...
        for temp_name_frame in mass_amp_in:
            ...
            amp_in = load_grayscale('Chisto_FREN/Archive/' + temp_name_frame,
                                    N)
            ...
            if show_img_on:
                cv2.imshow(
                    "Blue",
                    cv2.resize(
                        cv2.normalize(sum_image_B**2, None, 0, 255,
                                      cv2.NORM_MINMAX).astype(np.uint8),
                        (n_otobr, n_otobr)))
                cv2.imshow(
                    "Green",
                    cv2.resize(
                        cv2.normalize(sum_image_G**2, None, 0, 255,
                                      cv2.NORM_MINMAX).astype(np.uint8),
                        (n_otobr, n_otobr)))
                cv2.imshow(
                    "Red1",
                    cv2.resize(
                        cv2.normalize(sum_image_R**2, None, 0, 255,
                                      cv2.NORM_MINMAX).astype(np.uint8),
                        (n_otobr, n_otobr)))
                cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    mass_amp_in = [
        "400.png", "410.png", "420.png", "430.png", "440.png", "450.png",
        "460.png", "470.png", "480.png", "490.png", "500.png", "510.png",
        "520.png", "530.png", "540.png", "550.png", "560.png", "570.png",
        "580.png", "590.png", "600.png", "610.png", "620.png", "630.png",
        "640.png", "650.png", "660.png", "670.png", "680.png", "690.png",
        "700.png"
    ]

    lambda_for_lens = calc_for_lenz(n_prel, M, h)

    spectral_filters = get_filter_bier(lambda_for_lens,
                                       CMV_400_GRAPH_PATH,
                                       show_graph_on=True)
    main_circl(mass_amp_in, spectral_filters, dx_in, dx_lenz, dx_kam, r_lenz,
               focal_length, lambda_for_lens, z1, z2, N, True, 512, True)
